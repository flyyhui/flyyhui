...
5. 点击以下按钮将 Twikoo 一键部署到 Vercel（确保登录了 Vercel）
...
下面来认识 Twikoo 的管理面板，配置好上面的内容，就可以启动项目，然后看看效果，并在评论区的右边发现有一个「**蓝色小齿轮**」，这就是管理面板，**第一次** 点击小齿轮后会让你注册登录密码，请记住它，如果忘记了密码，请去直接的 MongoDB 数据库修改。
...
如果看到带有 **审核中** 的评论，则代表只有发言人和博主能看到，其他人无法看到，原因：

- 被 Akismet 认为是垃圾评论
- 博主隐藏了自己的评论，所以只能自己和博主看到
