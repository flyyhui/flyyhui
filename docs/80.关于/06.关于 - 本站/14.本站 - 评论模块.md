...
不使用评论区的页面：frontmatter 的 `comment` 为 false 的文章页、首页、归档页。

> 参数

第 5 行的 waitTime 变量：页面差不多渲染完后，多少毫秒后开始加载评论区，默认是 700 毫秒，时间太低可能导致页面没有完全渲染，就加载评论区，导致报错。

第 6 行的 archives 变量存放的是归档页的 permalink，主题默认是 `/archives/`，如果你曾修改了归档页的 permalink，请替换。
